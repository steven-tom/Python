...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while num_rolls>0:
        current_roll=dice()
        if current_roll!=1:
            dice_sum=dice_sum+current_roll
...
def reroll(dice):
    ...
    def rerolled():
        ...
        current_roll=dice()
        if current_roll%2==0:
            return current_roll  # Replace this statement
        else: 
            current_roll=dice()
            return current_roll
        # END PROBLEM 3
...
def make_averaged(fn, num_samples=1000):
    ...
    def avg_and_return(*args):
        total=0
        for i in range(1, num_samples+1):
            current_roll=fn(*args) #roll_dice(i,dice)
            total+=current_roll
            
        return total/num_samples
    return avg_and_return
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    for i in range(1,10+1):
        ...
        if  make_averaged(roll_dice)(i, dice) > score:
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    # BEGIN PROBLEM 10
    "*** REPLACE THIS LINE ***"
    #score includes hogtimus prime and free bacon already T_T
    if score*2!=opponent_score and hogtimus_prime(free_bacon(opponent_score))>= margin:
        return 0
    else:
        return num_rolls
    # END PROBLEM 10
